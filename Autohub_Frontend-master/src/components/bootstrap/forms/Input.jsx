...
const Input = forwardRef(
	(
		...
		const base =
			'w-full rounded-md border border-[var(--bs-border-color,#e5e7eb)] bg-white text-gray-900 dark:bg-slate-900 dark:text-slate-50 placeholder:text-gray-400 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-[var(--bs-primary,#6c5dd3)] disabled:cursor-not-allowed disabled:opacity-60';
		const sizeClass =
			size === 'sm'
				? 'h-9 px-3 text-sm'
				: size === 'lg'
		...
		const rangeClass =
			type === 'range'
				? 'h-2 w-full cursor-pointer accent-[var(--bs-primary,#6c5dd3)]'
				: '';

		const plaintextClass = readOnly === 'plaintext' ? 'border-0 bg-transparent px-0' : '';
		const colorClass = type === 'color' ? 'p-0 h-10 w-16' : '';
		...
		const _props = {
			...
			className: classNames(
				...
				plaintextClass,
				colorClass,
				!isValid && isTouched && invalidFeedback && 'ring-1 ring-red-500 focus-visible:ring-red-500',
				!isValid && isTouched && !invalidFeedback && 'ring-1 ring-green-500 focus-visible:ring-green-500',
				className,
			),
			required,
			placeholder,
