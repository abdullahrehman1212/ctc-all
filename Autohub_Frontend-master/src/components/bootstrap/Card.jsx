...
export const CardCodeView = React.memo(
	...
	({ children, language, customStyle, isPrismJs, className }) => {
		...
		const [hljsStyle, setHljsStyle] = useState(null);
		...
		useEffect(() => {
			...
			Promise.all([
				...
				if (!mounted) return;
				setSyntaxHighlighter(() => mod.default || mod);
				setHljsStyle(styles.atomOneLight || styles.default);
			});
			return () => {
				mounted = false;
			};
		...
		const resolvedStyle = useMemo(() => hljsStyle, [hljsStyle]);

		if (!SyntaxHighlighter || !resolvedStyle) {
			return (
				<pre
					className={classNames('shadow-sm overflow-auto', className)}
					style={{
						borderRadius: 13,
