...
const View = ({ tableDataLoading, tableData, refreshTableData }) => {
	...
	const [perPage, setPerPage] = useState(
		Number(store.data.itemsManagementModule.dimensions.perPage),
	);
	...
	const getEditingItem = (idd) => {
		...
		apiClient
			.get(`/editDimension?id=${idd}`)
			.then((res) => {
				setEditingItem(res.data.dimension);
	...
	useEffect(
		() => {
			dispatch(
				updateSingleState([perPage, 'itemsManagementModule', 'dimensions', 'perPage']),
			);
		},
	...
	const handlePageChange = (e) => {
		dispatch(updateSingleState([e, 'itemsManagementModule', 'dimensions', 'pageNo']));
	};
			...
			<CardBody className='table-responsive'>
				<table className='table table-modern'>
					...
					{tableDataLoading ? (
						...
						<tbody>
							{store.data.itemsManagementModule.dimensions.tableData?.data.map(
								(item, index) => (
									<tr key={item.id}>
										<td>
											<Checks
				...
				<PaginationButtons
					label='make'
					from={store.data.itemsManagementModule.dimensions.tableData?.from ?? 1}
					to={store.data.itemsManagementModule.dimensions.tableData?.to ?? 1}
					total={store.data.itemsManagementModule.dimensions.tableData?.total ?? 0}
				...
				<div className='row d-flex justify-content-end'>
					<div className='col-md-4'>
						<Pagination
							activePage={store.data.itemsManagementModule.dimensions?.pageNo ?? 1}
							totalItemsCount={
								store.data.itemsManagementModule.dimensions?.tableData?.total ?? 0
							}
							itemsCountPerPage={Number(
								store.data.itemsManagementModule.dimensions?.perPage ?? 10,
							)}
						...
						<CardHeader>
							<CardLabel icon='Delete' iconColor='info'>
								<CardTitle>
									...
									<small> Dimension Id: {itemId}</small>
								</CardTitle>
							</CardLabel>
						</CardHeader>
					</ModalTitle>
					...
					<div className='row g-4'>
						<div className='col-12'>
							<Card>
								<CardBody>
									<h5>
										Are you sure, you want to delete the selected Dimension?{' '}
										<br />
					...
					<ModalTitle id='editVoucher'>
						...
						<CardHeader>
							<CardLabel icon='Edit' iconColor='info'>
								<CardTitle>Editing Dimension</CardTitle>
								<small> Dimension Id: {itemId}</small>
							</CardLabel>
						</CardHeader>
					</ModalTitle>
