...
const View = ({ tableDataLoading, tableData, refreshTableData }) => {
	...
	const [perPage, setPerPage] = useState(
		Number(store.data.itemsManagementModule.applications.perPage),
	);
	...
	const getEditingItem = (idd) => {
		...
		apiClient
			.get(`/editApplication?id=${idd}`)
			.then((res) => {
				setEditingItem(res.data.application);
	...
	const deleteItem = (id) => {
		apiClient
			.delete(`/deleteApplication?id=${id}`)
			.then((res) => {
	...
	useEffect(
		() => {
			dispatch(
				updateSingleState([perPage, 'itemsManagementModule', 'applications', 'perPage']),
			);
		},
	...
	const handlePageChange = (e) => {
		dispatch(updateSingleState([e, 'itemsManagementModule', 'applications', 'pageNo']));
	};
			...
			<CardBody className='table-responsive'>
				<table className='table table-modern'>
					...
					{tableDataLoading ? (
						...
						<tbody>
							{/* {store.data.itemsManagementModule.applications.tableData?.data.map( */}
							{store.data.itemsManagementModule.applications.tableData?.data?.map(
								(item, index) => (
									<tr key={item.id}>
										<td>
				...
				<PaginationButtons
					label='make'
					from={store.data.itemsManagementModule.applications.tableData?.from ?? 1}
					to={store.data.itemsManagementModule.applications.tableData?.to ?? 1}
					total={store.data.itemsManagementModule.applications.tableData?.total ?? 0}
				...
				<div className='row d-flex justify-content-end'>
					<div className='col-md-4'>
						<Pagination
							activePage={store.data.itemsManagementModule.applications?.pageNo ?? 1}
							totalItemsCount={
								store.data.itemsManagementModule.applications?.tableData?.total ?? 0
							}
							itemsCountPerPage={Number(
								store.data.itemsManagementModule.applications?.perPage ?? 10,
							)}
						...
						<CardHeader>
							<CardLabel icon='Delete' iconColor='info'>
								<CardTitle>
									...
									<small> Application Id: {itemId}</small>
								</CardTitle>
							</CardLabel>
						</CardHeader>
					</ModalTitle>
					...
					<ModalTitle id='editVoucher'>
						...
						<CardHeader>
							<CardLabel icon='Edit' iconColor='info'>
								<CardTitle>Editing Application</CardTitle>
								<small> Application Id: {itemId}</small>
							</CardLabel>
						</CardHeader>
					</ModalTitle>
