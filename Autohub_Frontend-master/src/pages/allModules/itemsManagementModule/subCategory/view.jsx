...
const View = ({ tableDataLoading, tableData, refreshTableData }) => {
	...
	const [perPage, setPerPage] = useState(
		Number(store.data.itemsManagementModule.subcategory.perPage),
	);
	...
	const getEditingItem = (idd) => {
		...
		apiClient
			.get(`/editSubCategories?id=${idd}`)
			.then((res) => {
				setEditingItem(res.data.subcategory);
	...
	const deleteItem = (id) => {
		apiClient
			.delete(`/deleteSubCategories?id=${id}`)
			.then((res) => {
	...
	useEffect(
		() => {
			dispatch(
				updateSingleState([perPage, 'itemsManagementModule', 'subcategory', 'perPage']),
			);
		},
	...
	const handlePageChange = (e) => {
		dispatch(updateSingleState([e, 'itemsManagementModule', 'subcategory', 'pageNo']));
	};
			...
			<CardBody className='table-responsive'>
				<table className='table table-modern'>
					...
					{tableDataLoading ? (
						...
						<tbody>
							{store.data.itemsManagementModule.subcategory.tableData?.data?.map(
								(item, index) => (
									<tr key={item.id}>
										<td>
											<Checks
				...
				<PaginationButtons
					label='make'
					from={store.data.itemsManagementModule.subcategory.tableData?.from ?? 1}
					to={store.data.itemsManagementModule.subcategory.tableData?.to ?? 1}
					total={store.data.itemsManagementModule.subcategory.tableData?.total ?? 0}
				...
				<div className='row d-flex justify-content-end'>
					<div className='col-md-4'>
						<Pagination
							activePage={store.data.itemsManagementModule.subcategory?.pageNo ?? 1}
							totalItemsCount={
								store.data.itemsManagementModule.subcategory?.tableData?.total ?? 0
							}
							itemsCountPerPage={Number(
								store.data.itemsManagementModule.subcategory?.perPage ?? 10,
							)}
						...
						<CardHeader>
							<CardLabel icon='Delete' iconColor='info'>
								<CardTitle>
									...
									<small> SubCategory Id: {itemId}</small>
								</CardTitle>
							</CardLabel>
						</CardHeader>
					</ModalTitle>
					...
					<ModalTitle id='editVoucher'>
						...
						<CardHeader>
							<CardLabel icon='Edit' iconColor='info'>
								<CardTitle>Editing SubCategory</CardTitle>
								<small> SubCategory Id: {itemId}</small>
							</CardLabel>
						</CardHeader>
					</ModalTitle>
