...
const View = ({ tableDataLoading, tableData, refreshTableData }) => {
	...
	const [perPage, setPerPage] = useState(
		Number(store.data.itemsManagementModule.partModel.perPage),
	);
	...
	const getEditingItem = (idd) => {
		...
		apiClient
			.get(`/editmachinepartmodels?id=${idd}`)
			.then((res) => {
				setEditingItem(res.data.machinepartmodel);
	...
	const deleteItem = (id) => {
		apiClient
			.delete(`/deleteMachinePartModel?id=${id}`)
			.then((res) => {
	...
	useEffect(
		() => {
			dispatch(updateSingleState([perPage, 'itemsManagementModule', 'partModel', 'perPage']));
		},
	...
	const handlePageChange = (e) => {
		dispatch(updateSingleState([e, 'itemsManagementModule', 'partModel', 'pageNo']));
	};
			...
			<CardBody className='table-responsive'>
				<table className='table table-modern'>
					...
					{tableDataLoading ? (
						...
						<tbody>
							{store.data.itemsManagementModule.partModel.tableData?.data?.map(
								(item, index) => (
									<tr key={item.id}>
										<td>
											<Checks
				...
				<PaginationButtons
					label='make'
					from={store.data.itemsManagementModule.partModel.tableData?.from ?? 1}
					to={store.data.itemsManagementModule.partModel.tableData?.to ?? 1}
					total={store.data.itemsManagementModule.partModel.tableData?.total ?? 0}
				...
				<div className='row d-flex justify-content-end'>
					<div className='col-md-4'>
						<Pagination
							activePage={store.data.itemsManagementModule.partModel?.pageNo ?? 1}
							totalItemsCount={
								store.data.itemsManagementModule.partModel?.tableData?.total ?? 0
							}
							itemsCountPerPage={Number(
								store.data.itemsManagementModule.partModel?.perPage ?? 10,
							)}
					...
					<div className='row g-4'>
						<div className='col-12'>
							<Card>
								<CardBody>
									<h5>
										Are you sure, you want to delete the selected Part Model?{' '}
										<br />
					...
					<ModalTitle id='editVoucher'>
						...
						<CardHeader>
							<CardLabel icon='Edit' iconColor='info'>
								<CardTitle>Editing Part Model</CardTitle>
								<small> Part Model Id: {itemId}</small>
							</CardLabel>
						</CardHeader>
					</ModalTitle>
				</ModalHeader>
