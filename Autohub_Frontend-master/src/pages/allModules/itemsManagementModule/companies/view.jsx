...
const View = ({ tableDataLoading, tableData, refreshTableData }) => {
	...
	const [perPage, setPerPage] = useState(
		Number(store.data.itemsManagementModule.companies.perPage),
	);
	...
	const getEditingItem = (idd) => {
		...
		apiClient
			.get(`/editCompany?id=${idd}`)
			.then((res) => {
				setEditingItem(res.data.company);
	...
	useEffect(
		() => {
			dispatch(updateSingleState([perPage, 'itemsManagementModule', 'companies', 'perPage']));
		},
	...
	const handlePageChange = (e) => {
		dispatch(updateSingleState([e, 'itemsManagementModule', 'companies', 'pageNo']));
	};
			...
			<CardBody className='table-responsive'>
				<table className='table table-modern'>
					...
					{tableDataLoading ? (
						...
						<tbody>
							{store.data.itemsManagementModule.companies.tableData.data.map(
								(item, index) => (
									<tr key={item.id}>
										<td>
											<Checks
				...
				<div className='row d-flex justify-content-end'>
					<div className='col-md-4'>
						<Pagination
							...
							totalItemsCount={
								store.data.itemsManagementModule.companies?.tableData?.total ?? 0
							}
							itemsCountPerPage={Number(
								store.data.itemsManagementModule.companies?.perPage ?? 10,
							)}
					...
					<ModalTitle id='editVoucher'>
						...
						<CardHeader>
							<CardLabel icon='Edit' iconColor='info'>
								<CardTitle>Editing Company</CardTitle>
								<small> Item Id: {itemId}</small>
							</CardLabel>
						</CardHeader>
					</ModalTitle>
