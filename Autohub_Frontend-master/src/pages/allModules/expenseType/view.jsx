...
const View = ({ tableDataLoading, tableData, refreshTableData }) => {
	...
	const [perPage, setPerPage] = useState(
		Number(store.data.expenseManagementModule.expense.perPage),
	);
	...
	const getEditingItem = (idd) => {
		...
		apiClient
			.get(`/editExpenseType?id=${idd}`)
			.then((res) => {
				setEditingItem(res.data.expense_type);
	...
	const deleteItem = (id) => {
		apiClient
			.delete(`/deleteExpenseType?id=${id}`)
			.then((res) => {
	...
	useEffect(
		() => {
			dispatch(updateSingleState([perPage, 'expenseManagementModule', 'expense', 'perPage']));
		},
	...
	const handlePageChange = (e) => {
		dispatch(updateSingleState([e, 'expenseManagementModule', 'expense', 'pageNo']));
	};
			...
			<CardBody className='table-responsive'>
				<table className='table table-modern'>
					...
					{tableDataLoading ? (
						...
						<tbody>
							{store.data.expenseManagementModule.expense.tableData?.data?.map(
								(item, index) => (
									<tr key={item.id}>
										<td>
											<Checks
				...
				<PaginationButtons
					label='make'
					from={store.data.expenseManagementModule.expense.tableData?.from ?? 1}
					to={store.data.expenseManagementModule.expense.tableData?.to ?? 1}
					total={store.data.expenseManagementModule.expense.tableData?.total ?? 0}
				...
				<div className='row d-flex justify-content-end'>
					<div className='col-md-4'>
						<Pagination
							activePage={store.data.expenseManagementModule.expense?.pageNo ?? 1}
							totalItemsCount={
								store.data.expenseManagementModule.expense?.tableData?.total ?? 0
							}
							itemsCountPerPage={Number(
								store.data.expenseManagementModule.expense?.perPage ?? 10,
							)}
					...
					<div className='row g-4'>
						<div className='col-12'>
							<Card>
								<CardBody>
									<h5>
										Are you sure, you want to delete the selected Expense Type?{' '}
										<br />
					...
					<ModalTitle id='editVoucher'>
						...
						<CardHeader>
							<CardLabel icon='Edit' iconColor='info'>
								<CardTitle>Editing Expense Type</CardTitle>
								<small> Expense Id: {itemId}</small>
							</CardLabel>
						</CardHeader>
					</ModalTitle>
