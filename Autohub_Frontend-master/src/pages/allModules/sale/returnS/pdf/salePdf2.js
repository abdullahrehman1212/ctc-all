...
const generatePDF3 =  async (invoiceData2, type) => {
	let data = null;
	try {
	...
	const LogoSrc = data?.user?.logo_url !== 'NA' ? data?.user?.logo_url : Logo;
	let LogoBase64 = '';
	...
	yPos += 40;
	doc.addImage(LogoBase64, 'JPEG', 40, 30, 75, 50);
	...
	subTotalRows.push(['Deduction', `${invoiceData2?.parentData?.deduction}`]);
	...
	yPos += 10;
	const strLength = `PKR ${invoiceData2?.parentData?.total_after_discount?.toLocaleString(
		undefined,
		{
			maximumFractionDigits: 2,
		},
	...
	doc.setFont('Sans Serif');
	doc.text(
		`PKR ${invoiceData2?.parentData?.total_after_discount?.toLocaleString(undefined, {
			maximumFractionDigits: 2,
		})}/-`,
		pageWidth - 50,
		yPos,
	...
	yPos += 10;
	const strLength1 = `PKR ${invoiceData2?.parentData?.gst?.toLocaleString(undefined, {
		maximumFractionDigits: 2,
	})}/-`.length;
	doc.setFontSize(8);
	doc.setFont('Helvetica', 'bold');
	...
	doc.setFont('Sans Serif');
	doc.text(
		`PKR ${invoiceData2?.parentData?.gst?.toLocaleString(undefined, {
			maximumFractionDigits: 2,
		})}/-`,
		pageWidth - 50,
		yPos,
	...
	yPos += 10;
	const strLength2 = `PKR ${invoiceData2?.parentData?.total_after_gst?.toLocaleString(undefined, {
		maximumFractionDigits: 2,
	})}/-`.length;
	doc.setFontSize(8);
	doc.setFont('Helvetica', 'bold');
	...
	doc.setFont('Sans Serif');
	doc.text(
		`PKR ${invoiceData2?.parentData?.total_after_gst?.toLocaleString(undefined, {
			maximumFractionDigits: 2,
		})}/-`,
		pageWidth - 50,
		yPos,
