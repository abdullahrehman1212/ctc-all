...
const generatePDF2 = (invoiceData, type) => {
	let data3 = null;
	try {
	...
	yPos += 10;
	const strLength3 =
		`PKR ${invoiceData?.parentData?.customer?.coa_account2.balance?.balance.toLocaleString(
			undefined,
			{
				maximumFractionDigits: 2,
			},
	...
	doc.setFontSize(9);
	doc.setFont('Sans Serif');
	doc.text(`${invoiceData?.parentData?.delivered_to}`, pageWidth - 480, yPos, { align: 'left' });
	yPos += 20;
	doc.setFontSize(10);
	doc.setFont('Helvetica', 'bold');
