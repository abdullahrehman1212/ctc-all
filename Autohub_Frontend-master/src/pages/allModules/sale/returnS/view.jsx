...
import originalInvoicePdf from './pdf/salePdf';
import printreturnInvoicePdf from './pdf/salePdf2';
...
const View = ({ tableDataLoading, tableData, refreshTableData }) => {
	...
	const [centeredStatusInvoice2, setCenteredStatusInvoice2] = useState(false);
	const [invoiceModal2, setInvoiceModal2] = useState(false);
	const [invoiceDataLoading2, setInvoiceDataLoading2] = useState(false);
	...
	const getInvoiceData2 = (id) => {
		setInvoiceDataLoading2(true);
		apiClient
			.get(`getReturnedSalesByid?id=${id}`)
			.then((response) => {
				setInvoiceData2(response.data);

				setInvoiceDataLoading2(false);
			})
			.catch((err) => {
				// showNotification(_titleError, err.message, 'Danger');
				if (err.response.status === 401) {
	...
	const deleteItem = (id) => {
		apiClient
			.delete(`/deletereturnedsale?id=${id}`)
			.then((res) => {
	...
	return (
		<>
									...
									<tr key={item.id}>
										...
										<td>
											...
											<div className='small text-muted'>
												{item?.invoice?.store?.store_type?.name}
											</div>
										</td>
										<td>{item?.invoice?.remarks}</td>
										<td>
										...
										<td>
											{item?.invoice?.discount?.toLocaleString(undefined, {
												maximumFractionDigits: 2,
											})}
										</td>
										<td>
											{item?.invoice?.total_after_discount?.toLocaleString(
												undefined,
												{
													maximumFractionDigits: 2,
												},
										...
										{/* <td>{item?.invoice.received_amount}</td> */}
										...
										<td>
											<Icon
												...
												color='primary'
												onClick={() => {
													setInvoiceModal2(true);
													getInvoiceData2(item.id);
												}}
											/>
										</td>
					...
					<div className='row g-4'>
						<div className='col-12'>
							<Card>
								<CardBody>
									<h5>
										Are you sure, you want to delete the selected Return Sale
										Order? <br />
			...
			<Modal
				...
				<ModalHeader setIsOpen={headerCloseStatusReturn1 ? setStateReturn1 : null}>
					<ModalTitle id='ReturnToSaleOrder'>
						...
						<CardHeader>
							<CardLabel icon='Edit' iconColor='info'>
								<CardTitle>Return To Sale Order {poNo1}</CardTitle>
							</CardLabel>
						</CardHeader>
					</ModalTitle>
				</ModalHeader>
					...
					<CardFooter>
						...
						<CardFooterRight>
							<Button
								...
								color='primary'
								onClick={() => {
									originalInvoicePdf(invoiceData, 2);
								}}>
			...
			{/* Invoice Modal2 */}
			<Modal
				isOpen={invoiceModal2}
				setIsOpen={setInvoiceModal2}
				isCentered={centeredStatusInvoice2}
				titleId='exampleModalLabel'
				size='xl'>
				<ModalHeader setIsOpen={headerCloseStatusEdit ? setInvoiceModal2 : null}>
					<ModalTitle id='editVoucher2'>
						{' '}
						<CardHeader>
							<CardLabel icon='Preview' iconColor='info'>
								<CardTitle>
									Return Invoice Details <br />
									<small>
										{' '}
										Invoice Number: {invoiceData2?.parentData?.inv_id}
									</small>
								</CardTitle>
							</CardLabel>
						</CardHeader>
				...
				<ModalBody>
					<Card className='p-5' stretch>
						{invoiceDataLoading2 ? (
							...
							<ViewInvoice2 invoiceData2={invoiceData2} />
						)}
					</Card>
					<CardFooter>
						<CardFooterLeft>
							<Button
								...
								isOutline
								className='border-0'
								onClick={() => setInvoiceModal2(false)}>
								Close
							</Button>
						</CardFooterLeft>
						<CardFooterRight>
							<Button
								...
								color='primary'
								onClick={() => {
									printreturnInvoicePdf(invoiceData2, 2);
								}}>
								PRINT
							</Button>
						</CardFooterRight>
