...
const generatePDF2 = async (invoiceData1, type) => {
	let data = null;
	try {
	...
	// rightTableRows.push([`Invoice No.: ${invoiceData1.parantData.invoice_no}`]);
	rightTableRows.push([`Invoice : ${invoiceData1?.parentData?.invoice_no}`]);

	rightTableRows.push([
		`Date: ${
			invoiceData1.parentData.date
				? moment(invoiceData1.parentData.date).format('DD-MM-YYYY')
				: ''
		}`,
	]);

	// rightTableRows.push([`Time: ${invoiceData1?.parentData?.created_at?.slice(11, 19)}`]);
	...
	vendorTableRows2.push([
		`Name: ${
			invoiceData1?.parentData.sale_type === 1
				? invoiceData1?.parentData.walk_in_customer_name
				: invoiceData1?.parentData.customer?.name
		}`,
	]);
	vendorTableRows2.push([
		`Contact: ${
			invoiceData1?.parentData.sale_type === 1
				? ''
				: invoiceData1?.parentData.customer?.phone_no
		}`,
	]);
	...
	invoiceData1?.childData?.forEach((item, index) => {
	...
	subTotalRows.push(['Discount', `${invoiceData1?.parentData?.discount}`]);
	...
	yPos += 10;
	const strLength = `PKR ${invoiceData1?.parentData?.total_after_discount?.toLocaleString(
		undefined,
		{
			maximumFractionDigits: 2,
		},
	...
	doc.setFont('Sans Serif');
	doc.text(
		`PKR ${invoiceData1?.parentData?.total_after_discount?.toLocaleString(undefined, {
			maximumFractionDigits: 2,
		})}/-`,
		pageWidth - 50,
		yPos,
	...
	yPos += 10;
	const strLength1 = `PKR ${invoiceData1?.parentData?.gst?.toLocaleString(undefined, {
		maximumFractionDigits: 2,
	})}/-`.length;
	doc.setFontSize(8);
	doc.setFont('Helvetica', 'bold');
	...
	doc.setFont('Sans Serif');
	doc.text(
		`PKR ${invoiceData1?.parentData?.gst?.toLocaleString(undefined, {
			maximumFractionDigits: 2,
		})}/-`,
		pageWidth - 50,
		yPos,
	...
	yPos += 10;
	const strLength2 = `PKR ${invoiceData1?.parentData?.total_after_gst?.toLocaleString(undefined, {
		maximumFractionDigits: 2,
	})}/-`.length;
	doc.setFontSize(8);
	doc.setFont('Helvetica', 'bold');
	...
	doc.setFont('Sans Serif');
	doc.text(
		`PKR ${invoiceData1?.parentData?.total_after_gst?.toLocaleString(undefined, {
			maximumFractionDigits: 2,
		})}/-`,
		pageWidth - 50,
		yPos,
	...
	yPos += 10;
	const strLength3 =
		`PKR ${invoiceData1?.parentData?.customer?.coa_account2.balance.balance.toLocaleString(
			undefined,
			{
				maximumFractionDigits: 2,
			},
	...
	doc.setFont('Sans Serif');
	doc.text(
		`${invoiceData1?.parentData?.delivered_to ? invoiceData1?.parentData?.delivered_to : ''}`,
		pageWidth - 480,
		yPos,
		{ align: 'left' },
	);
