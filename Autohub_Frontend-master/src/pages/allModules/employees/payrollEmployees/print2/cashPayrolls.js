...
const GeneratePDF = async (data1, dateOfPayroll, status, Account, ChequeNo, type) => {
	...
	data1.forEach((data) => {
		if (
			data.payment_mode === 'Cash Payment' &&
			Number(data.employee.employee_payable_account.balance.balance) < 0
		) {
			totalPayable += Number(
	...
	doc.text('Payroll Payment Mode: Cash Payment', pageWidth / 2, 55, { align: 'center' });
