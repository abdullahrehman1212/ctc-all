...
const validate = (values) => {
	...
	if (values.working_days < 0) {
		errors.working_days = 'Must not be less than zero';
	}
	if (values.working_days > values.employee_details.employee_profile.working_days) {
		errors.working_days = 'Must not be more Total Working Days';
	}
...
const AddCoeSubGroup = (props) => {
	...
	const submitForm = (myFormik) => {
		...
		showNotification(_titleSuccess, 'Saved temporarily', 'success');
		// setState(false);
		myFormik.resetForm();
	...
	useEffect(() => {
		formik.setFieldValue(
			'basic_salary',
			(
				(Number(formik.values.employee_details.employee_profile.basic_salary) /
					Number(formik.values.employee_details.employee_profile.working_days)) *
				Number(formik.values.working_days)
			).toFixed(),
		);
	...
	useEffect(() => {
		const StartDate = moment(formik.values.payrole_month, 'YYYY-MM-DD')
			.startOf('month')
			.format('DD/MM/YY');
		const discharge = moment(
			moment(formik.values.leaving_date, 'DD-MM-YYYY').format('DD-MM-YYYY'),
			'DD-MM-YYYY',
		);
		const admission = moment(moment(StartDate, 'DD/MM/YY').format('DD-MM-YYYY'), 'DD-MM-YYYY');
		const perTotal =
			((discharge.diff(admission, 'days') + 1) /
				moment(StartDate, 'DD/MM/YY').daysInMonth()) *
			100;
				...
				<CardBody>
					<div className='row g-4'>
						<CardBody className='col-md-6 border-end'>
							<div className='row g-4'>
								<CardLabel className='col-md-6' icon='Money' iconColor='danger'>
									<CardTitle>
