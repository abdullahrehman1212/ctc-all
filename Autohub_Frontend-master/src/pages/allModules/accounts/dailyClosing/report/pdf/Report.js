...
const GeneratePDF = (data1, type, specificItem, startDate, endDate) => {
				...
				if (item2.invoice_child.length > 0) {
					...
					item2.invoice_child.forEach((item3) => {
						if (item3.invoice) {
							...
							totalCostPart +=
								item3.cost * (item3.quantity - item3.returned_quantity);
							totalProfitPart +=
								item3.price * (item3.quantity - item3.returned_quantity) -
								item3.cost * (item3.quantity - item3.returned_quantity);
							// Calculate Ends
							const itemsData3 = [
								count3,
								item3.invoice?.store.name,
	...
	doc.text('Brand Wise Sales Report', pageWidth / 2, 55, { align: 'center' });
	doc.text(
