...
const EditModernPage = (props) => {
	...
	const [landPaymentHeadsType3, setLandPaymentHeadsType3] = useState([]);
	const [landPaymentHeadsLoadingType3, setLandPaymentHeadsLoadingType3] = useState(true);
	...
	useEffect(() => {
		...
		apiClient
			...
			.catch((err) => console.log(err));
		apiClient
			.get(`/getLandPaymentHeads?type=2`)
			.then((response) => {
			...
			.catch((err) => console.log(err));
		apiClient
			.get(`/getLandPaymentHeads?type=3`)
			.then((response) => {
				const rec = response.data.landPaymentHead.map(({ id, name }) => ({
					id,
					value: id,
				...
				setLandPaymentHeadsType3(rec);
				setLandPaymentHeadsLoadingType3(false);
			})
			// eslint-disable-next-line no-console
			.catch((err) => console.log(err));
	}, []);
									...
									<div className='col-md-3'>
										<FormGroup
											...
											<Select
												...
												isClearable
												isLoading={
													formik.values.list[0].land_payment_head_id
														.type === 2
														? landPaymentHeadsLoading
														: landPaymentHeadsLoadingType3
												}
												options={
													formik.values.list[0].land_payment_head_id
														.type === 2
														? landPaymentHeads
														: landPaymentHeadsType3
												}
												isDisabled={
													formik.values.list[0].land_payment_head_id
														.type !== 2
