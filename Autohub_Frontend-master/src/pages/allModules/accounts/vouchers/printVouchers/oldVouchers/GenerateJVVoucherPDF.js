...
const GeneratePDF = (data1, type) => {
	...
	data1.voucher_transactions.forEach((data) => {
		...
		const dataLand = [
			...
			data.credit === 0 ? `${data.coa_account.name}` : `    ${data.coa_account.name}`,
	...
	doc.text(data3?.user?.company_name, 240, 40);
	doc.setFontSize(12);
	...
	doc.text(` ${data1.id}`, 500, yPos);
	yPos += 25;

	doc.text(`Date`, 460, yPos);
	doc.text(`__________`, 490, yPos);
	...
	yPos += 20;
	doc.text(
		`  Being the adjustment of _______________________________________________________ `,
		45,
		yPos,
	);
	yPos += 50;
