...
const GeneratePDF = (data1, type, startDate, endDate) => {
	...
	data1.forEach((item) => {
		...
		if (item.invoice && item.invoice.length > 0 && item.invoice[0]?.invoice_child?.length > 0) {
			count += 1;
			...
			item.invoice.forEach((item2) => {
				if (item2.invoice_child.length > 0) {
					...
					tableRows.push([
						...
						{
							content: `Store: ${item2.store.name}`,
							colSpan: 1,
							rowSpan: 1,
							styles: {
								halign: 'left',
					...
					item2.invoice_child.forEach((item3) => {
						...
						const itemsData2 = [
							...
							(item3.price * item3.quantity)?.toLocaleString(undefined, {
								maximumFractionDigits: 2,
							}) ?? 0,
							// item3.returned_quantity?.toLocaleString(undefined, {
							// 	maximumFractionDigits: 2,
							// }) ?? 0,
							item3.cost?.toLocaleString(undefined, {
								maximumFractionDigits: 2,
							}) ?? 0,
							(
								item3.price * item3.quantity -
								item3.cost * item3.quantity
			...
			tableRows.push([
				{
					content: `Customer Summary: ${item.name}`,
					colSpan: 13,
					rowSpan: 1,
					styles: {
						halign: 'center',
			...
			tableRows.push([
				...
				{
					content: `${item.coa_account2.balance.balance.toLocaleString(undefined, {
						maximumFractionDigits: 2,
					})}`,
					colSpan: 6,
					rowSpan: 1,
