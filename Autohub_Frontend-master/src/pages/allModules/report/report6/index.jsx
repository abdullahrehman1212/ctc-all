...
const Categories = () => {
	...
	const printReportAll = (docType) => {
		...
		apiClient
			.get(
				`/getSalesReportCustomerWise?records=${store.data.reportManagementModule.report6.perPage}&pageNo=${store.data.reportManagementModule.report6.pageNo}&colName=id&sort=desc&customer_id=${formik.values.customer_id}&store_id=${nameOptions.id}&brand_id=${formik.values.brand_id}&from=${startDate3}&to=${endDate3}`,
			)
			.then((response) => {
