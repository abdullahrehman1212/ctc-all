...
const Categories = () => {
	...
	const printReportAll = (docType) => {
		...
		apiClient
			.get(
				`/getSalesReportBySaleType?records=${store.data.reportManagementModule.report7.perPage}&pageNo=${store.data.reportManagementModule.report7.pageNo}&colName=id&sort=desc&sale_type=${formik.values.sale_type}&customer_id=${formik.values.customer_id}&walk_in_customer_name=${formik.values.walk_in_customer_name}&store_id=${nameOptions.id}&item_id=${formik.values.machine_part_id}&from=${startDate3}&to=${endDate3}`,
			)
			.then((response) => {
						...
						<Card>
							<CardHeader>
								<CardLabel icon='Assignment'>
									<CardTitle> Sales Type Report </CardTitle>
								</CardLabel>
							</CardHeader>
							<CardBody>
