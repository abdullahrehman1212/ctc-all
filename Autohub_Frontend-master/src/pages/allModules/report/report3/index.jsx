...
const Categories = () => {
	...
	const printReportAll = (docType) => {
		...
		apiClient
			.get(
				`/getPurchaseReport?records=${store.data.reportManagementModule.report1.perPage}&pageNo=${store.data.reportManagementModule.report1.pageNo}&colName=id&sort=desc&po_type=1&supplier_id=${formik.values.supplier_id}&store_id=${nameOptions.id}&from=${startDate3}&to=${endDate3}`,
			)
			.then((response) => {
				GeneratePDF(response.data.purchaseorderlist, docType, startDate2, endDate2);
				showNotification(
						...
						<Card>
							<CardHeader>
								<CardLabel icon='Assignment'>
									<CardTitle> Purchase Orders Expenses report </CardTitle>
								</CardLabel>
							</CardHeader>
							<CardBody>
