...
const View = ({ tableDataLoading, tableData, refreshTableData }) => {
	...
	const [perPage, setPerPage] = useState(
		Number(store.data.customersManagementModule.manage.perPage),
	);
	...
	useEffect(
		() => {
			dispatch(
				updateSingleState([perPage, 'customersManagementModule', 'manage', 'perPage']),
			);
		},
	...
	const handlePageChange = (e) => {
		dispatch(updateSingleState([e, 'customersManagementModule', 'manage', 'pageNo']));
	};
			...
			<CardBody className='table-responsive'>
				<table className='table table-modern'>
					...
					{tableDataLoading ? (
						...
						<tbody>
							{store.data.customersManagementModule.manage.tableData?.map(
								(item, index) => (
									<tr key={item.id}>
										<td>
											<Checks
				...
				<PaginationButtons
					label='make'
					from={store.data.customersManagementModule.manage.tableData?.from ?? 1}
					to={store.data.customersManagementModule.manage.tableData?.to ?? 1}
					total={store.data.customersManagementModule.manage.tableData?.total ?? 0}
				...
				<div className='row d-flex justify-content-end'>
					<div className='col-md-4'>
						<Pagination
							activePage={store.data.customersManagementModule.manage?.pageNo ?? 1}
							totalItemsCount={
								store.data.customersManagementModule.manage?.tableData?.total ?? 0
							}
							itemsCountPerPage={Number(
								store.data.customersManagementModule.manage?.perPage ?? 10,
							)}
					...
					<ModalTitle id='editVoucher'>
						...
						<CardHeader>
							<CardLabel icon='Edit' iconColor='info'>
								<CardTitle>Editing Customer</CardTitle>
								<small> Item Id: {itemId}</small>
							</CardLabel>
						</CardHeader>
					</ModalTitle>
