...
const generatePDF = (purchaseDetails, type) => {
	...
	doc.text('SIMPLE PURCHASE ORDER', pageWidth - 50, yPos + 10, { align: 'right' });
	doc.setTextColor('#000');
	...
	// 	`Address: ${
	// 		purchaseDetails?.purchaseorderlist?.supplier?.address === null
	// 			? 'none'
	...
	if (type === 1) {
		doc.save(`Simple Purchase Order Details ${moment()}`);
	} else if (type === 2) {
