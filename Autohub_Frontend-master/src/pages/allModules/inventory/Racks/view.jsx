...
const RacksView = ({ tableDataLoading, tableData, refreshTableData }) => {
	console.log('table Data is:', tableData);
	// const navigate = useNavigate();
	const dispatch = useDispatch();
	const store = useSelector((state) => state.tableCrud);
	const [perPage, setPerPage] = useState(
		Number(store.data.inventoryManagementModule.racks.perPage),
	);
	...
	const getEditingItem = (idd) => {
		...
		apiClient
			.get(`/editRack?id=${idd}`)
			.then((res) => {
				setEditingItem(res.data.rack);
				console.log('editing item here:', res.data.rack);
	...
	useEffect(
		() => {
			dispatch(updateSingleState([perPage, 'inventoryManagementModule', 'rack', 'perPage']));
		},
	...
	const handlePageChange = (e) => {
		dispatch(updateSingleState([e, 'inventoryManagementModule', 'rack', 'pageNo']));
	};
			...
			<CardBody className='table-responsive'>
				<table className='table table-modern'>
					...
					{tableDataLoading ? (
						...
						<tbody>
							{console.log(store.data.inventoryManagementModule.racks.tableData?.data)}
							{store.data.inventoryManagementModule?.racks?.tableData?.data.map(
								(item, index) => (
									<tr key={item.id}>
										<td>
											<Checks
				...
				<PaginationButtons
					label='make'
					from={store.data.inventoryManagementModule.racks.tableData?.from ?? 1}
					to={store.data.inventoryManagementModule.racks.tableData?.to ?? 1}
					total={store.data.inventoryManagementModule.racks.tableData?.total ?? 0}
				...
				<div className='row d-flex justify-content-end'>
					<div className='col-md-4'>
						<Pagination
							activePage={store.data.inventoryManagementModule.racks?.pageNo ?? 1}
							totalItemsCount={
								store.data.inventoryManagementModule.racks?.tableData?.total ?? 0
							}
							itemsCountPerPage={Number(
								store.data.inventoryManagementModule.racks?.perPage ?? 10,
							)}
					...
					<div className='row g-4'>
						<div className='col-12'>
							<Card>
								<CardBody>
									<h5>
										Are you sure, you want to delete the selected Rack? <br />
					...
					<ModalTitle id='editVoucher'>
						...
						<CardHeader>
							<CardLabel icon='Edit' iconColor='info'>
								<CardTitle>Editing Rack</CardTitle>
								<small> Rack Id: {itemId}</small>
							</CardLabel>
						</CardHeader>
					</ModalTitle>
