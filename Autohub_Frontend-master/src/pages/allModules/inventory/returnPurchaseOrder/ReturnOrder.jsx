...
const ReturnOrder = ({ returnData, handleStateReturn }) => {
	...
	useEffect(() => {
		...
		apiClient
			...
			.then((response) => {
				...
				const rec = response.data.po_no_dropdown.map(({ id, po_no }) => ({
					id,
					value: id,
	...
	const loadPOData = async (val) => {
		if (val) {
			setPoLoading(true);
			try {
				const response = await apiClient.get(`/editPurchaseOrder?id=${val.id}`);
	...
	const formik = useFormik({
		initialValues: { ...po, deduction: '', data: '' },
													...
													<div>
														<FormGroup
															...
															<Input
																...
																isValid={formik.isValid}
																isTouched={
																	formik.touched.returned_quantity
																}
																invalidFeedback={
