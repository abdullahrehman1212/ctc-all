...
const generatePDF = (invoiceData, type) => {
	...
	rightTableRows.push([`PO No. : ${invoiceData?.purchaseorderlist?.po_no}`]);
	rightTableRows.push([
		`Status : ${
			// eslint-disable-next-line no-nested-ternary
			invoiceData?.purchaseorderlist?.is_approve === 0 &&
			invoiceData?.purchaseorderlist?.is_received === 0
				? 'Pending'
				: invoiceData?.purchaseorderlist?.is_approve === 1 &&
				  invoiceData?.purchaseorderlist?.is_received === 0
				? 'Approved'
				: 'Received'
		}`,
