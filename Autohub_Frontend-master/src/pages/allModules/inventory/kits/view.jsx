...
const View = ({ tableDataLoading, tableData, refreshTableData }) => {
	...
	useEffect(
		() => {
			dispatch(updateSingleState([perPage, 'inventoryManagementModule', 'kits', 'perPage']));
		},
	...
	const handlePageChange = (e) => {
		dispatch(updateSingleState([e, 'inventoryManagementModule', 'kits', 'pageNo']));
	};
	...
	return (
		<>
			<CardBody className='table-responsive'>
				...
				<PaginationButtons
					label='kits'
					from={store.data.inventoryManagementModule.kits.tableData?.from ?? 1}
					to={store.data.inventoryManagementModule.kits.tableData?.to ?? 1}
					total={store.data.inventoryManagementModule.kits.tableData?.total ?? 0}
				...
				<div className='row d-flex justify-content-end'>
					<div className='col-md-4'>
						<Pagination
							activePage={store.data.inventoryManagementModule.kits?.pageNo ?? 1}
							totalItemsCount={
								store.data.inventoryManagementModule.kits?.tableData?.total ?? 0
							}
							itemsCountPerPage={Number(
								store.data.inventoryManagementModule.kits?.perPage ?? 10,
							)}
					...
					<div className='row g-4'>
						<div className='col-12'>
							<Card>
								<CardBody>
									<h5>
										Are you sure, you want to delete the selected Kit Inventory?{' '}
										<br />
					...
					<ModalTitle id='editVoucher'>
						...
						<CardHeader>
							<CardLabel icon='Edit' iconColor='info'>
								<CardTitle>Editing Inventory Kit</CardTitle>
								<small> Item Id: {itemId}</small>
							</CardLabel>
						</CardHeader>
					</ModalTitle>
