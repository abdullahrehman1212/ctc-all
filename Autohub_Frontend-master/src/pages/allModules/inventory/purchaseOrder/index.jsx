...
const Categories = () => {
	...
	useEffect(() => {
		...
		apiClient
			...
			.then((response) => {
				...
				// console.log('The response in purchase is:', rec);
	...
	const PrintReportPurchasedItems = (docType) => {
		...
		apiClient
			.get(`/getPoChild?type_id=${formik.values.type_id ? formik.values.type_id : ''}&po_no=${po}&item_id=${
					formik.values.machine_part_id ? formik.values.machine_part_id : ''
				}&supplier_id=${selectedItem ? selectedItem.id : ''}`)
			.then((response) => {
				console.log(response.data.pochild, docType,"response.data.pochild, docType")
