...
const generateInvoicePDF = (purchaseDetails, type) => {
	...
	rightTableRows.push([
		...
		purchaseDetails?.purchaseorderlist?.is_approved === 0 &&
			purchaseDetails?.purchaseorderlist?.received_date &&
			moment(purchaseDetails?.purchaseorderlist?.received_date).isValid() &&
			purchaseDetails?.purchaseorderlist?.received_date !== '1899-11-29T19:17:10.000Z'
			? moment(purchaseDetails?.purchaseorderlist?.received_date).format('DD/MM/YYYY')
			: 'Not Received'
		}`,
	]);
