...
const GeneratePDF = (data1, type) => {
	...
	data1?.originalPurchaseOrder?.childData.forEach((item, index) => {
		totalAmountOriginalPo += Number(item.purchase_price * item.received_quantity);
		...
		const itemsData = [
			...
			(item.purchase_price * item.received_quantity).toLocaleString(undefined, {
	...
	data1.originalPurchaseOrder?.poHistory.forEach((item) => {
		...
		item?.return_purchase_order_child.forEach((item3) => {
			...
			const itemsData = [
				...
				item3.purchase_price.toLocaleString(undefined, {
					maximumFractionDigits: 2,
				}) || '',
				(item3.returned_quantity * item3.purchase_price).toLocaleString(undefined, {
					maximumFractionDigits: 2,
				}) ?? 0,
			];
	...
	yPos += 15;
	doc.text(`Returned PO History`, pageWidth / 2, yPos, { align: 'center' });
	yPos += 5;
