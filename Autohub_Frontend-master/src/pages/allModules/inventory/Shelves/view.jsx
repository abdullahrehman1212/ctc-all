...
const View = ({ tableDataLoading, tableData, refreshTableData }) => {
	console.log('Shelf Data Contents:', tableData);
	// const navigate = useNavigate();
	const dispatch = useDispatch();
	const store = useSelector((state) => state.tableCrud);
	const [perPage, setPerPage] = useState(
		Number(store.data.inventoryManagementModule.shelves.perPage),
	);
	...
	const getEditingItem = (idd) => {
		...
		apiClient
			.get(`/editShelves?id=${idd}`)
			.then((res) => {
				setEditingItem(res.data.shelves);
	...
	useEffect(
		() => {
			dispatch(
				updateSingleState([perPage, 'inventoryManagementModule', 'shelves', 'perPage']),
			);
		},
	...
	const handlePageChange = (e) => {
		dispatch(updateSingleState([e, 'inventoryManagementModule', 'shelves', 'pageNo']));
	};
			...
			<CardBody className='table-responsive'>
				<table className='table table-modern'>
					...
					{tableDataLoading ? (
						...
						<tbody>
							{store.data.inventoryManagementModule.shelves.tableData?.data?.map(
								(item, index) => (
									<tr key={item.id}>
										<td>
											<Checks
				...
				<PaginationButtons
					label='make'
					from={store.data.inventoryManagementModule.shelves.tableData?.from ?? 1}
					to={store.data.inventoryManagementModule.shelves.tableData?.to ?? 1}
					total={store.data.inventoryManagementModule.shelves.tableData?.total ?? 0}
				...
				<div className='row d-flex justify-content-end'>
					<div className='col-md-4'>
						<Pagination
							activePage={store.data.inventoryManagementModule.shelves?.pageNo ?? 1}
							totalItemsCount={
								store.data.inventoryManagementModule.shelves?.tableData?.total ?? 0
							}
							itemsCountPerPage={Number(
								store.data.inventoryManagementModule.shelves?.perPage ?? 10,
							)}
					...
					<div className='row g-4'>
						<div className='col-12'>
							<Card>
								<CardBody>
									<h5>
										Are you sure, you want to delete the selected SubCategory?{' '}
										<br />
					...
					<ModalTitle id='editVoucher'>
						...
						<CardHeader>
							<CardLabel icon='Edit' iconColor='info'>
								<CardTitle>Editing Shelves</CardTitle>
								<small> Shelves Id: {itemId}</small>
							</CardLabel>
						</CardHeader>
					</ModalTitle>
				</ModalHeader>
