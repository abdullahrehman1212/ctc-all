...
import { Scale, Calendar, Printer, FileText } from "lucide-react";
...
// Assets data - starts empty
const assetsData: AccountBalance[] = [];

// Liabilities data - starts empty
const liabilitiesData: AccountBalance[] = [];
...
export const BalanceSheetReport = () => {
  ...
  const totalLiabilities = liabilitiesData.filter(a => !a.isSubgroup && !a.isTotal).reduce((sum, a) => sum + a.balance, 0);
      ...
      <CardContent className="p-6 space-y-6">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
          ...
          <div className="border rounded-md">
            ...
            <div className="px-2">
              {liabilitiesData.map((item, index) => renderAccountRow(item, index))}
              ...
              <div className="flex justify-between items-center py-2 border-b border-border/40" style={{ paddingLeft: '72px' }}>
                ...
                <span className="font-mono text-sm">{formatNumber(0)}</span>
              </div>
              <div className="flex justify-between items-center py-2 border-b border-border/40 font-semibold" style={{ paddingLeft: '48px' }}>
                <span className="text-primary">Total 501-Owner Equity</span>
                <div className="flex">
                  <span className="font-mono text-sm text-primary min-w-[100px] text-right mr-6">{formatNumber(0)}</span>
                  <span className="font-mono text-sm text-primary min-w-[100px] text-right">{formatNumber(0)}</span>
                </div>
              </div>
              <div className="flex justify-between items-center py-2 border-b border-border/40" style={{ paddingLeft: '48px' }}>
                ...
                <span className={`font-mono text-sm ${ownerEquity < 0 ? 'text-destructive' : ''}`}>
                  {formatNumber(ownerEquity)}
                </span>
              </div>
              ...
              <div className="flex justify-between items-center py-3 border-t border-foreground/30 font-bold" style={{ paddingLeft: '24px' }}>
                ...
                <span className={`font-mono ${ownerEquity < 0 ? 'text-destructive' : ''}`}>
                  {formatNumber(ownerEquity)}
                </span>
              </div>
