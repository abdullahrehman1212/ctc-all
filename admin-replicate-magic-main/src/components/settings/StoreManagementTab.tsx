...
export const StoreManagementTab = () => {
  ...
  const handleDeleteStore = (id: string) => {
    // Delete associated racks and shelves
    const storeRacks = racks.filter(r => r.storeId === id);
    const rackIds = storeRacks.map(r => r.id);
    setShelves(shelves.filter(s => !rackIds.includes(s.rackId)));
    setRacks(racks.filter(r => r.storeId !== id));
  ...
  const handleRackSubmit = () => {
    ...
    if (editingRack) {
      setRacks(racks.map(r => r.id === editingRack.id ? {
        ...r,
        codeNo: rackFormData.codeNo,
        description: rackFormData.description,
        status: rackFormData.status,
      } : r));
  ...
  const handleDeleteRack = (rackId: string) => {
    setShelves(shelves.filter(s => s.rackId !== rackId));
  ...
  const handleShelfSubmit = () => {
    ...
    if (editingShelf) {
      setShelves(shelves.map(s => s.id === editingShelf.id ? {
        ...s,
        shelfNo: shelfFormData.shelfNo,
        description: shelfFormData.description,
        status: shelfFormData.status,
      } : s));
