...
const categories = ["All Categories", "Engine Parts", "Transmission", "Suspension", "Electrical", "Brake System", "Uncategorized", "Male", "boster", "engine"];
const stores = ["All Stores", "Main Store", "Store A", "Store B"];
...
export const StockBalance = () => {
  ...
  const storeStock = useMemo(() => {
    ...
    filteredData.forEach(item => {
      storeMap[item.store] = (storeMap[item.store] || 0) + item.quantity;
    });
    return Object.entries(storeMap).sort((a, b) => b[1] - a[1]);
  }, [filteredData]);
    ...
    <div className="space-y-4">
      ...
      <div className="bg-card rounded-lg border border-border overflow-hidden">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between px-3 py-2 border-b border-border gap-2">
          ...
          <p className="text-xs text-muted-foreground">Showing {filteredData.length} of {sampleData.length} items</p>
        </div>
