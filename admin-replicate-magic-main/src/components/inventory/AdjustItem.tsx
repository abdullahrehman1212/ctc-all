...
const sampleItems: { id: string; name: string; qtyInStock: number; lastPurchaseRate: number }[] = [];

const stores = ["Main Warehouse", "Store A", "Store B", "Store C"];
const categories = ["Electronics", "Mechanical", "Consumables", "Tools"];
...
export const AdjustItem = () => {
  ...
  const handleItemChange = (id: string, field: keyof AdjustmentItem, value: string | number) => {
    setAdjustmentItems(adjustmentItems.map(item => {
      if (item.id === id) {
        ...
        if (field === "itemId") {
          const selectedItem = sampleItems.find(si => si.id === value);
          if (selectedItem) {
            updated.itemName = selectedItem.name;
            updated.qtyInStock = selectedItem.qtyInStock;
            updated.lastPurchaseRate = selectedItem.lastPurchaseRate;
          }
        }
  ...
  const handleSave = () => {
    ...
    if (!hasValidItems) {
      toast({
        title: "Validation Error",
        description: "Please add at least one item with quantity and rate",
        variant: "destructive",
      });
      return;
    }
    ...
    const newRecord: AdjustmentRecord = {
      ...
      store,
      addInventory,
      items: adjustmentItems.filter(item => item.quantity > 0),
      notes,
      totalAmount
    };
    ...
    if (view === "edit" && selectedRecord) {
      ...
      toast({ title: "Success", description: "Adjustment updated successfully" });
    } else {
  ...
  const handleView = (record: AdjustmentRecord) => {
    setRecordToView(record);
    setViewDialogOpen(true);
  };

  const handleEdit = (record: AdjustmentRecord) => {
    setSelectedRecord(record);
    setAddInventory(record.addInventory);
    setDate(record.date.split("/").reverse().join("-"));
    setSubject(record.subject);
    setStore(record.store);
    setNotes(record.notes);
    setAdjustmentItems(record.items.length > 0 ? record.items : [
      { id: "1", itemId: "", itemName: "", qtyInStock: 0, quantity: 0, lastPurchaseRate: 0, rate: 0, total: 0 }
    ]);
    setView("edit");
  };
  ...
  const handleConfirmDelete = () => {
    if (recordToDelete) {
      ...
      toast({ title: "Deleted", description: "Adjustment deleted successfully" });
              ...
              <div className="space-y-3">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  <div>
                    ...
                    <p className="text-xs font-medium">{recordToView.id}</p>
                  </div>
                  <div>
          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <div className="flex items-center gap-1.5">
                  <Label className="text-xs whitespace-nowrap">Sub Category</Label>
                    ...
                    <TableRow key={item.id}>
                      <TableCell>
                        ...
                        >
                          ...
                          <SelectContent>
                            {sampleItems.map((si) => (
                              <SelectItem key={si.id} value={si.id} className="text-xs">{si.name}</SelectItem>
                            ))}
                          </SelectContent>
                        </Select>
                      </TableCell>
